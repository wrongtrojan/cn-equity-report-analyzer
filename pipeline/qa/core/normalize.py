...
from pipeline.item_aliases import ITEM_ALIASES, expand_item_names
...
def apply_intent_rules(
    ...
    if re.search(r"分别|以及|和.*多少|哪些方面|整体情况|综合性|营收.*利润|利润.*营收", q):
        if _NUMERIC_HINTS.search(q) or re.search(r"怎么样|如何|概况", q):
            period_labels = [str(year)] if year else list(normalized.sql_targets.period_labels)
            item_names = _extract_item_names(q, list(normalized.sql_targets.item_names), list(normalized.entities))
            if not item_names:
                item_names = ["营业总收入", "归属于上市公司股东的净利润"]
            return normalized.model_copy(
                update={
                    "intent": "hybrid",
                    "section_keys": ["mda", "key_financials", "financial_statements"],
                    "vector_query": vector_query,
                    "sql_targets": SQLTargets(
                        ...
                        period_kinds=["year"],
                        stmt_types=["kpi", "income"],
                        period_granularity=reconcile_granularity(
                            normalized.sql_targets.period_granularity, q
                        ),
                    ),
                }
            )

    if re.search(r"请.*介绍|公司概况|基本情况|整体介绍|基本信息|介绍.*(公司|基本|概况|整体)|公司.*(基本|概况|信息)", q):
        if not _NUMERIC_HINTS.search(q):
            return normalized.model_copy(
                update={
                    "intent": "hybrid",
                    "section_keys": ["company_profile", "mda", "key_financials"],
                    "vector_query": vector_query,
                    "sql_targets": SQLTargets(
                        item_names=["营业总收入", "归属于上市公司股东的净利润"],
                        period_labels=[str(year)] if year else [],
                        period_kinds=["year"],
                        stmt_types=["kpi"],
                        period_granularity="annual",
                    ),
    ...
    if re.search(r"主要业务|业务是什么|业务有哪些|从事.*业务", q):
        if not _NUMERIC_HINTS.search(q):
            return normalized.model_copy(
                update={
                    "intent": "narrative",
    ...
    if re.search(r"注册地址|注册地|办公地址|住所|总部在哪|地址在哪", q):
        return normalized.model_copy(
            update={
                "intent": "narrative",
                "section_keys": ["company_profile"],
                "vector_query": vector_query,
            }
        )

    if re.search(r"股票代码|证券代码|股票简称", q):
        return normalized.model_copy(
            update={
                "intent": "narrative",
                "section_keys": ["company_profile"],
    ...
    if re.search(r"股东|持股|控股|实际控制人|第一大股东|子公司|关联方|董事|监事|高管|董事长|监事会主席", q):
